# 📘 Day 16 — IOC Enrichment & Risk Classification Pipeline

## Objective

Build a modular Python-based enrichment pipeline that:

* Reads an external IOC feed (simulating SIEM/XDR telemetry)
* Matches IOCs against a local intelligence lookup database
* Enriches matched indicators with contextual data (country, malware family, first seen date)
* Assigns a risk level based on confidence and malware attribution
* Produces structured, readable output

## File 1 — `ioc_lookup.json` (Local Intelligence Database)
...
## File 2 — `external_feed.json` (Incoming Feed)
